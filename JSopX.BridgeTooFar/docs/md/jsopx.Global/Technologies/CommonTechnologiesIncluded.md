...
It is assumed that all JSopX™ SaaS Projects _require_ some common technologies that are included in all projects by default.
...
The following _JSopX™ SaaS Projects Collection_ Projects that depend on these `Common Technologies Included` are as follows:
...
**Purpose:** Cross-platform MAUI Hybrid App and Web Multi App Library (_Windows, Android, iOS, macOS, and Web_).
...
**Purpose:** A first-party ReactCore for Jason Silvestri that starts small (Contacts → Orgs → Interactions) but is **device-ubiquitous**  (_Windows, Android, iOS, macOS, and Web_).
...
**Purpose:** A first-party VueCore (Product Lifecycle Management) system for Jason Silvestri that starts small (Products → Lifecycles → Lifecycle Phases → Workflows → Decision Gates) but is **device-ubiquitous** (_Windows, Android, iOS, macOS, and Web_).
